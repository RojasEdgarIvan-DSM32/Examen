    ...
    <div class="container mx-auto p-4">
        ...
        <form action="{{ route('personas.update', $persona->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre" id="nombre" value="{{ $persona->nombre }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
            </div>
            <div class="mb-4">
                <label for="direccion" class="block text-sm font-medium text-gray-700">Dirección</label>
                <input type="text" name="direccion" id="direccion" value="{{ $persona->direccion }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="municipio_id" id="municipio_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    @foreach($municipios as $municipio)
                        <option value="{{ $municipio->id }}" {{ $persona->municipio_id == $municipio->id ? 'selected' : '' }}>
                            {{ $municipio->nombre }}
                        </option>
                    @endforeach
                </select>
